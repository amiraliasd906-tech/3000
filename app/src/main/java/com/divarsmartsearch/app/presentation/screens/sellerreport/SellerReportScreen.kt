    ...
    LazyColumn(
        ...
        item {
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    ...
                    // Bug fix, per explicit user request: this used to show
                    // a "میانگین احتمال واسطه بودن" percentage derived from
                    // ownerProbability. That field is no longer a real,
                    // independent signal — it now just reflects whether the
                    // person's OWN keyword filters happened to match, so
                    // showing it here as if it were an assessment of the
                    // seller was misleading. Removed entirely; the ad-count
                    // warning below is a genuine, independent signal and
                    // stays as-is.
        ...
        items(report.listings, key = { it.id }) { listing ->
            ...
            Card(
                ...
                modifier = Modifier.fillMaxWidth().pressDepth3D(pressed = isPressed),
            ) {
