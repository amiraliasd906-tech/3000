...
fun DivarNavGraph() {
    ...
    Scaffold(
        ...
        NavHost(
            ...
            // Depth-style transitions: the incoming screen scales up out of
            // the distance while fading in, the outgoing one recedes/fades
            // — a lightweight "3D push" feel between screens instead of the
            // previous instant, flat swap. Reversed on back-navigation so it
            // reads as coming back toward the camera.
            enterTransition = {
            ...
            exitTransition = {
                fadeOut(animationSpec = tween(200)) +
                    scaleOut(targetScale = 1.06f, animationSpec = tween(200))
            },
            popEnterTransition = {
                fadeIn(animationSpec = tween(260)) +
                    scaleIn(initialScale = 1.06f, animationSpec = tween(260))
            },
            popExitTransition = {
                fadeOut(animationSpec = tween(200)) +
