...
object DatabaseModule {
    ...
    fun provideAppDatabase(@ApplicationContext context: Context): AppDatabase =
        Room.databaseBuilder(context, AppDatabase::class.java, AppDatabase.DATABASE_NAME)
            .fallbackToDestructiveMigration() // See AppDatabase's KDoc: kept only as a
            // last-resort fallback for devices still on the old, un-recorded
            // schema versions (1–4). No migration is silently skipped for
            // the CURRENT version (5) since nothing has changed since it was
            // set — the moment a future change bumps the version, a real
            // Migration belongs here (added via .addMigrations(...)) instead
            // of relying on this to paper over it with a data wipe.
