 ...
 *
 * Real gap found: `exportSchema` was `false` and no [androidx.room.migration.Migration]
 * had ever been written, while [com.divarsmartsearch.app.di.DatabaseModule]
 * used `.fallbackToDestructiveMigration()` — meaning ANY future bump of
 * `version` (adding a column, a new entity, etc.) would silently wipe every
 * saved search, listing, filter, and history row instead of upgrading them.
 * `exportSchema = true` now captures a JSON snapshot of this exact schema
 * (version 5) under app/schemas/ on the next build — that snapshot is what
 * makes it possible to write a real, verified `Migration(5, 6)` (etc.) the
 * next time an entity changes, instead of guessing at ALTER TABLE
 * statements against a schema nobody recorded. Versions 1–4 were never
 * captured, so a device still on one of those can't be losslessly migrated
 * either way — the destructive fallback still applies there — but from
 * version 5 onward there is no excuse to lose data on a schema change.
 */
@Database(
    entities = [
